...
const { authMiddleware, superviseurOnly } = require('../middleware/auth');
...
const BASE_SELECT = `
  SELECT d.*,
    ...
    ag.nom || ' ' || COALESCE(ag.prenom,'') agent_nom
  FROM dossiers d
  JOIN clients c ON d.client_id = c.id
  JOIN type_declaration td ON d.type_decl_id = td.id
  LEFT JOIN utilisateurs u ON d.created_by = u.id
  LEFT JOIN utilisateurs ag ON d.agent_id = ag.id`;

router.get('/', async (req, res) => {
  try {
    const { q='', statut='', type='', page=1, limit=50, client_id, agent_id } = req.query;
    ...
    if (req.user.role === 'client') {
      ...
      conditions.push("(d.agent_id = ? OR d.created_by = ?)");
    ...
    if (agent_id)  { conditions.push("d.agent_id = ?"); params.push(agent_id); }
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Restriction client
    if (req.user.role === 'client') {
      if (!req.user.client_id || dossier.client_id !== req.user.client_id) {
        return res.status(403).json({ error: 'Accès non autorisé à ce dossier' });
      }
    }

    // Pour le client, ne charger que les étapes validées
    let etapesQuery = `
      SELECT e.*, (u.nom || ' ' || COALESCE(u.prenom,'')) agent_nom
      FROM etapes_dossier e
      LEFT JOIN utilisateurs u ON e.agent_id = u.id
      WHERE e.dossier_id = ?
    `;
    if (req.user.role === 'client') {
      etapesQuery += ` AND e.statut = 'validee' ORDER BY e.date_validation ASC, e.date_declaration ASC`;
    } else {
      etapesQuery += ` ORDER BY e.date_declaration ASC`;
    }

    const [notes, pieces, debours, factures, preavis, etapes] = await Promise.all([
      ...
      all(etapesQuery, [req.params.id])
    ]);

    const formattedEtapes = etapes.map(e => ({
      ...e,
      pieces_jointes: e.pieces_jointes ? JSON.parse(e.pieces_jointes) : []
    }));

    res.json({ ...dossier, notes, pieces, debours, factures, preavis, etapes: formattedEtapes });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/', async (req, res) => {
  try {
    const { client_id, type_decl_id, agent_id, description, marchandise, pays_origine,
    ...
    const r = await run(
      `INSERT INTO dossiers (reference,client_id,type_decl_id,agent_id,description,marchandise,pays_origine,pays_destination,valeur_douane,devise,incoterm,observations,created_by)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [reference, client_id, type_decl_id, agent_id||null, description||null, marchandise||null,
...
router.put('/:id', async (req, res) => {
  try {
    const { client_id, type_decl_id, agent_id, statut, description, marchandise, pays_origine,
            pays_destination, valeur_douane, devise, incoterm, observations } = req.body;
    await run(
      `UPDATE dossiers SET client_id=?,type_decl_id=?,agent_id=?,statut=?,description=?,marchandise=?,
       pays_origine=?,pays_destination=?,valeur_douane=?,devise=?,incoterm=?,observations=?,updated_at=datetime('now') WHERE id=?`,
      [client_id, type_decl_id, agent_id||null, statut||'ouvert', description||null, marchandise||null,
       pays_origine||null, pays_destination||null, valeur_douane||0, devise||'TND',
...
router.patch('/:id/assign', superviseurOnly, async (req, res) => {
  try {
    const { agent_id } = req.body;
    await run("UPDATE dossiers SET agent_id=?, updated_at=datetime('now') WHERE id=?", [agent_id || null, req.params.id]);
    res.json({ message: 'Agent attribué au dossier' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
